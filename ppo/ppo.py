...
class PPOTrainer:
    ...
    def __init__(self, E: Embedder, **ppo_params):
        """
        ...
        self.optimizer = Adam(E.parameters(), lr=self.ppo_params['lr'])
    ...
    def batched_forward_pass(self, queries, responses):
        ...
        for i in range(int(bs / fbs)):
            ...
            #     "input_ids"]
            t_response_batch = nn.utils.rnn.pad_sequence(response_batch, batch_first=True,
                                                         padding_value=self.model.dataset.c2i['<pad>'])
            t_query_batch = torch.cat([t.unsqueeze(0) for t in query_batch], dim=0)
            with torch.no_grad():
                _, logits, v = self.model.forward_decoder(response_batch, t_query_batch, isRef=False, addValue=True)
                _, ref_logits = self.model.forward_decoder(response_batch, t_query_batch, isRef=True, addValue=False)
            logprobs = logprobs_from_logits(logits[:, :-1, :], t_response_batch[:, 1:])
            ref_logprobs = logprobs_from_logits(ref_logits[:, :-1, :], t_response_batch[:, 1:])
            for j in range(fbs):
                end = len(response_batch[j])
                all_values.append(v[j, :end-1])  # TODO, maybe add zero on start and remove last
                all_logprobs.append(logprobs[j, :end])
                all_ref_logprobs.append(ref_logprobs[j, :end])
    ...
    def loss(self, old_logprobs, values, rewards, query, response):
        ...
        lastgaelam = 0
        advantages_reversed = []
        gen_len = response.shape[1] - 1 # first one is bos
        ...
        _, logits, vpred = self.model.forward_decoder((response.squeeze(0),), query, addValue=True)
